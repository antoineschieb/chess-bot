...
templates = [['R',rook_template],['N',knight_template],['B',bishop_template],['Q',queen_template],['K',king_template],['P',pawn_template] ]
i=0
for t in templates: #t = ['R', rook_template]

    autres_pieces = [p[:2] for p in templates if p[0]!=t[0]] # = [['N',knight_template],['B',bishop_template],['Q',queen_template],['K',king_template],['P',pawn_template] ]
...
def simplify_line_str(line):
    ...
    for c in range(len(line) - 1):
        if line[c+0].isnumeric() and line[c+1].isnumeric():
            simplified_line = line[:c]+ str(int(line[c+0]) + int(line[c+1])) + line[c+2:]
            break
