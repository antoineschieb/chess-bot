...
def generate_fen_from_cropped_im(im, color_to_play="w"):
    ...
    for row in range(8):
        ...
        for col in range(8):
            ...
            squareRGB[ww * row : ww * (row + 1), ww * col : ww * (col + 1)] = (
                255,
                0,
                0,
            )
        ...
        if row == 0:
            if (
                virtual_line[0] == "r" and virtual_line[4] == "k"
            ):  # black can queenside castle
                castling_rights.append("q")
            if (
                virtual_line[-1] == "r" and virtual_line[4] == "k"
...
def take_screenshot_and_crop_it(window_name="Live Chess - Chess.com - Google Chrome"):
    ...
    if im:
        ...
        return cv2.cvtColor(
            im[x1:x2, y1:y2], cv2.COLOR_BGR2GRAY
        )  # giga important (zoom 110% si jamais) [haut,bas,gauche,droite]
    else:
